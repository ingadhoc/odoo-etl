...
class manager(osv.osv):
    ...
    _columns = {
        ...
        'migration_notes': fields.html(string='Migration Note'),
        ...
        'action_ids': fields.one2many('oerp_migrator.action', 'manager_id', string='Actions', domain=[('state','in',['to_analyze','enabled'])]), 
        'external_model_ids': fields.one2many('oerp_migrator.external_model', 'manager_id', string='External Models', readonly=True), 
        'value_mapping_field_ids': fields.one2many('oerp_migrator.value_mapping_field', 'manager_id', string='Value Mapping Fields'), 
    }
