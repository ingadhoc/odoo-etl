...
class field_mapping(osv.osv):
    ...
    _name = 'oerp_migrator.field_mapping'
    ...
    _columns = {
        ...
        'source_field_id': fields.many2one('oerp_migrator.field', string='Source Field'),
        'source_field': fields.char(string='Source Exp.'),
        'target_field_id': fields.many2one('oerp_migrator.field', string='Target Field'),
        'target_field': fields.char(string='Target Exp.'),
        'expression': fields.text(string='Expression'),
        'value_mapping_field_id': fields.many2one('oerp_migrator.value_mapping_field', string='Value Mapping Field'),
        'model_field_id': fields.many2one('oerp_migrator.field', string='Model Field'),
        'migration_notes': fields.html(string='Migration Notes'),
        'action_id': fields.many2one('oerp_migrator.action', string='Action', ondelete='cascade', required=True), 
    }
