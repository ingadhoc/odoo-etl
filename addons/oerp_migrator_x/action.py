...
class action(osv.osv):
    ...
    _columns = {
        ...
        'source_model_id': fields.many2one('oerp_migrator.external_model', ondelete='cascade', string='source_model_id', required=True), 
        'target_model_id': fields.many2one('oerp_migrator.external_model', ondelete='cascade', string='target_model_id'),         
    }
    ...
    def match_fields(self, cr, uid, ids, context=None):
        ...
        for action in self.browse(cr, uid, ids):
            ...
            for field in migrator_field_obj.browse(cr, uid, source_field_ids, context=context):
                ...
                value_mapping_ids = self.pool['oerp_migrator.value_mapping_field'].search(cr, uid, [('name','=',field.name),('manager_id','=',action.manager_id.id)], context=context)
    ...
    def updata_records_number(self, cr, uid, ids, source_connection=False, target_connection=False, context=None):
        ...
        for action in self.browse(cr, uid, ids, context=context):
            if not source_connection or not target_connection:
                (source_connection, target_connection) = self.pool.get('oerp_migrator.manager').open_connections(cr, uid, [action.manager_id.id], context=context)
        ...
        for action in self.browse(cr, uid, ids, context=context):
            ...
            for readed_record, source_data_record in zip(source_data_date_adapt, source_model_data):
                ...
                for field_value, source_mapping in zip(readed_record, source_mapping_date_adapt):
                    if source_mapping.source_field_id.ttype == 'datetime':
                    ...
                    if source_mapping.source_field_id.ttype == 'date':
