...
class action(osv.osv):
    ...
    _columns = {
        ...
        'source_model_id': fields.many2one('elt.external_model', ondelete='cascade', string='source_model_id', required=True), 
        'target_model_id': fields.many2one('elt.external_model', ondelete='cascade', string='target_model_id'),         
    }
    ...
    def match_fields(self, cr, uid, ids, context=None):
        ''' Mach fields'''
        migrator_field_obj = self.pool.get('elt.field') 
        field_mapping_obj = self.pool.get('elt.field_mapping')        
        ...
        for action in self.browse(cr, uid, ids):
            ...
            for field in migrator_field_obj.browse(cr, uid, source_field_ids, context=context):
                ...
                value_mapping_ids = self.pool['elt.value_mapping_field'].search(cr, uid, [('name','=',field.name),('manager_id','=',action.manager_id.id)], context=context)
    ...
    def updata_records_number(self, cr, uid, ids, source_connection=False, target_connection=False, context=None):
        migrator_model_obj = self.pool.get('elt.external_model')
        for action in self.browse(cr, uid, ids, context=context):
            if not source_connection or not target_connection:
                (source_connection, target_connection) = self.pool.get('elt.manager').open_connections(cr, uid, [action.manager_id.id], context=context)
    ...
    def run_action(self, cr, uid, ids, source_connection=False, target_connection=False, repeated_action=False, context=None):
        ...
        migrator_model_obj = self.pool.get('elt.external_model')
        field_mapping_obj = self.pool.get('elt.field_mapping')
        value_mapping_field_detail_obj = self.pool.get('elt.value_mapping_field_detail')
        value_mapping_field_obj = self.pool.get('elt.value_mapping_field')
        for action in self.browse(cr, uid, ids, context=context):
            if not source_connection or not target_connection:
                (source_connection, target_connection) = self.pool.get('elt.manager').open_connections(cr, uid, [action.manager_id.id], context=context)
            ...
            source_fields.extend([x.source_field for x in action.field_mapping_ids if x.state==state and x.type == 'field'])

            # target fields = enabled and field then expression then migrated_id
            target_fields.extend([x.target_field for x in action.field_mapping_ids if x.state==state and x.type == 'field'])
            ...
            # Read and append source values of type 'field'
                ...
                for field_value, value_mapping_id in zip(readed_record, source_value_mapping_id):
                    ...
                    if value_mapping.type == 'id':
                        ...
                        new_field_ids = value_mapping_field_detail_obj.search(cr, uid, [('source_value_id.ext_id','=',field_value),
                            ('value_mapping_field_id','=',value_mapping_id)], context=context)
                        print 'new_field_ids', new_field_ids
                        if new_field_ids:
            ...
            print'target_model_data', target_model_data
