...
class manager(osv.osv):
    ...
    def run_actions(self, cr, uid, ids, context=None):
        ...
        action_obj = self.pool.get('elt.action')
        for i in ids:
    ...
    def match_models(self, cr, uid, ids, context=None):
        ...
        elt_model_obj = self.pool.get('elt.external_model') 
        action_obj = self.pool.get('elt.action') 
        for manager in self.browse(cr, uid, ids):
            print 'Matching models for manager ', manager.name
            # read all source models
            source_domain = [('manager_id','=',manager.id),('type','=','source')]
            source_model_ids = elt_model_obj.search(cr, uid, source_domain, context=context)
            ...
            for model in elt_model_obj.browse(cr, uid, source_model_ids, context=context):
                target_model_id = False
                target_domain = [('manager_id','=',manager.id),('type','=','target'),('model','=',model.model)]
                target_model_ids = elt_model_obj.search(cr, uid, target_domain, context=context)
    ...
    def get_records(self, cr, uid, ids, context=None):    
        ...
        for manager in self.browse(cr, uid, ids):
            (source_connection, target_connection) = self.open_connections(cr, uid, [manager.id], context=context)
            source_model_ids = elt_model_obj.search(cr, uid, [('type','=','source'),('manager_id','=',manager.id)], context=context)
            target_model_ids = elt_model_obj.search(cr, uid, [('type','=','target'),('manager_id','=',manager.id)], context=context)
            elt_model_obj.get_records(cr, uid, source_model_ids, source_connection, context=context)
            elt_model_obj.get_records(cr, uid, target_model_ids, target_connection, context=context)
        return True 

    def read_models(self, cr, uid, ids, context=None):  
        '''Get models and fields of source and target database for manager ids'''
        external_model_obj = self.pool['elt.external_model']
        for manager in self.browse(cr, uid, ids):
            (source_connection, target_connection) = self.open_connections(cr, uid, [manager.id], context=context)
    ...
    def read_model(self, cr, uid, connection, relation_type, manager_id, context=None):     
        ...
        rec_ids = elt_model_obj.load(cr, uid, external_model_fields, external_model_data, context=None)        
        return rec_ids  