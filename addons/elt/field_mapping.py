...
class field_mapping(osv.osv):
    ...
    _columns = {
        ...
        'source_field_id': fields.many2one('elt.field', string='Source Field'),
        'source_field': fields.char(string='Source Exp.'),
        'target_field_id': fields.many2one('elt.field', string='Target Field'),
        'target_field': fields.char(string='Target Exp.'),
        'expression': fields.text(string='Expression'),
        'value_mapping_field_id': fields.many2one('elt.value_mapping_field', string='Value Mapping Field'),
        'model_field_id': fields.many2one('elt.field', string='Model Field'),
        'note': fields.html(string='Notes'),
        'action_id': fields.many2one('elt.action', string='Action', ondelete='cascade', required=True), 
    }
