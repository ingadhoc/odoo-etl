...
class manager(osv.osv):
    ...
    _columns = {
        ...
        'action_ids': fields.one2many('elt.action', 'manager_id', string='Actions', domain=[('state','in',['to_analyze','enabled'])]), 
        'external_model_ids': fields.one2many('elt.external_model', 'manager_id', string='External Models', readonly=True), 
        'value_mapping_field_ids': fields.one2many('elt.value_mapping_field', 'manager_id', string='Value Mapping Fields'), 
    }
