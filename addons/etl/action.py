...
class action(models.Model):
    ...
    def check_m2o_depends(self, cr, uid, ids, context=None):
        ...
        for action in self.browse(cr, uid, ids):
            ...
            future_action_ids = self.search(cr, uid, [('manager_id','=',self.manager_id.id),('sequence','>=',self.sequence),('state','in',['enabled','to_analyze'])], context=context)
            future_actions = []
            for future_action in self.browse(cr, uid, future_action_ids, context=context):
                future_actions.append(future_self.source_model_id.model)
            
            # Look for active fields of this action
            field_mapping_domain = [('blocked','!=',True),('action_id','=',self.id),('source_field_id.ttype','=', 'many2one'),('state','in',['enabled','to_analyze','on_repeating',]),('type','=','field')]
            ...
            for mapping in field_mapping_obj.browse(cr, uid, field_mapping_ids, context=context):
                ...
                if dependency in future_actions:
                    state = 'on_repeating'
                    repeating_action = True
                    vals = ['field_mapping_' + str(self.id) + '_' + str(mapping.source_field_id.id), state,]
                    data.append(vals) 
            fields = ['id', 'state']
            ...
            self.write(cr, uid, [self.id], vals, context=context)                       
    ...
    def updata_records_number(self, cr, uid, ids, source_connection=False, target_connection=False, context=None):
        ...
        for action in self.browse(cr, uid, ids, context=context):
            if not source_connection or not target_connection:
                (source_connection, target_connection) = action.pool.get('etl.manager').open_connections(cr, uid, [self.manager_id.id], context=context)
